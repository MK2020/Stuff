...
while(infile1 >> tmp1x >> tmp1y >> tmp2x >> tmp2y >> tmp3x >> tmp3y){
	...
	Point tmp1(tmp1x, tmp1y);
	Point tmp2(tmp2x, tmp2y);
	Point tmp3(tmp3x,tmp3y);
...
//does this version without iterators work?
